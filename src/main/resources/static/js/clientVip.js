...
layui.use([ 'jquery', 'table', 'layer', 'form', 'laydate' ],
	...
	//工具栏事件
	  table.on('toolbar(test)', function(obj){
		  var checkStatus = table.checkStatus('clientVip');
	      ...
	      case 'deleteMore':
	    	  if (data.length == 0) {
		        ...
		        layer.confirm('真的删除'+data.length+'条数据么',function(){
		        	$.ajax({
			        	url:"/lms/clientVip/deleteMore?ids="+ids,
			        	type:'post',
			        	dataType:'json',
		  ...
		  table.on('tool(test)', function(obj){
		    ...
		    if(obj.event === 'del'){
			      layer.confirm('真的删除行么', function(index){
			        ...
			        $.ajax({
							url : "/lms/clientVip/deleteById?id="+data.id,
							type : "POST",
							dataType : 'json',
							success : function(data) {
								if (data.statue == 0) {
